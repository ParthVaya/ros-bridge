...
def costmap_callback(data, tf_listener):
    global costmap
    ...
    # Apply thresholding: values 128 or above are set to 255, others to 0
    _, thresholded_costmap = cv2.threshold(costmap_cv, 127, 255, cv2.THRESH_BINARY)

    # Get the transformation
    (trans, rot) = tf_listener.lookupTransform('ego_vehicle', data.header.frame_id, rospy.Time(0))
    ...
    #warped_costmap = eye_bird_view(cropped_costmap, mtx, dist, d=530)

    # Resize 
    resized_costmap = cv2.resize(cropped_costmap, (1280, 720))

    costmap = resized_costmap
    ...
    print("Size of resized_costmap:", costmap.shape)
    print(costmap)
    cv2.imshow("Cropped Local Costmap", costmap)
    cv2.waitKey(1)'''
...
def processing_mask(mask, img, show=False, d=530):
    ...
    #print("Shape of line_edges:", line_edges.shape)
    #print("Shape of costmap:", costmap.shape)

    if costmap is not None:
        line_edges = cv2.bitwise_or(line_edges, costmap)
...
def image_callback(data):
    ...
    '''print("Type of line_edges:", type(line_edges))
    print("Type of costmap:", type(costmap))
    print("Shape of line_edges:", line_edges.shape)
    print("Shape of costmap:", costmap.shape)
    print("Data type of line_edges:", line_edges.dtype)
    print("Data type of costmap:", costmap.dtype)
    print(line_edges[300:310, 700:710])'''

    steer_msg.data = degree_steering_angle
    steer_pub.publish(steer_msg)

if __name__ == '__main__':
    rospy.init_node('carla_path_planning_plus1', anonymous=True)
    bridge = CvBridge()
